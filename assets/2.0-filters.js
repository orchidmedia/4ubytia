...
class FacetRemove extends HTMLElement {
  constructor() {
    ...
    removeFilterNode.addEventListener('click', e => {
      ...
      filtersForm.renderPage(new URL(removeFilterNode.href).searchParams.toString(), true, true);
    });
  }
...
class CollectionFiltersForm extends HTMLElement {
  constructor() {
      ...
      this.listeners = [events_addEventDelegate({
        ...
        handler: (e, option) => {
          if (option !== this.activeSortingOption) {
            ...
            AmyrisTheme.Collection.closeMobileSorting();
    ...
    _defineProperty(this, "onHistoryChange", event => {
      ...
      this.renderPage(searchParams, false, false);
    });

    _defineProperty(this, "renderPage", (searchParams, updateURLHash = true, outsideForm = false) => { //, event = null
      this.loading.start();
      ...
      renderFunc(_url).then(() => {
        AmyrisTheme.Collection.init();
        const modalFilter = document.querySelector('.sf__modal-filter');
        if (!outsideForm) {
          if (window.location.href.indexOf("filter") > -1) {
            modalFilter.style.display = 'flex';
          }
          if (window.innerWidth <= 991 && modalFilter) {
            if (window.location.href.indexOf("filter") > -1) {
    ...
    this.debouncedOnSubmit = debounce(event => {
      this.onSubmitHandler(event, event.target.name === "sort_by");
    }, 500);
  ...
  onSubmitHandler(event, oustideForm = false) {
    event.preventDefault();
    const formData = new FormData(this.domNodes.filtersForm);
    const searchParams = new URLSearchParams(formData).toString();
    this.renderPage(searchParams, true, oustideForm);
  }
