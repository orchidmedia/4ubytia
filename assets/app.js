...
Shopify.onCartUpdate = async function (cart, open_drawer = true) {
  try {
    ...
    if (Cart) {
      ...
      if (window.beautyLabsCart && window.beautyLabsCart.onIncoming) {
        window.beautyLabsCart.onIncoming(cart)
      }
      ...
      cart.item_count < 1 ? $('.shipping_messages, .checkout_top').hide() : $('.shipping_messages, .checkout_top').show();
      cart.item_count < 1 ? $('.shipping_messages, .checkout_bottom').hide() : $('.shipping_messages, .checkout_bottom').show();

      cart.item_count < 1 ? $('.sf-cart-drawer form').addClass('h-full') : $('.sf-cart-drawer form').removeClass('h-full');

      var msgWrapper = document.querySelector('.shipping_messages');
      var shipping_value = Number(msgWrapper.dataset.threshold * 100);
      var cartTotal = cart.total_price;
      var shipping_value_left = Number(shipping_value - cartTotal);
      var progress = Number(100 - shipping_value_left/100*2)

      if(cartTotal < shipping_value){
        msgWrapper.innerHTML = `
        You're <span class="shipping_value mx-1">${Shopify.formatMoney(shipping_value_left, '${{amount}}')}</span>${msgWrapper.dataset.shippingTxt}
        <progressBar style="--shipping-progress:${ progress }%"><span>${ progress }</span></progressBar>`;
      }else{
        msgWrapper.innerHTML = `<b class="qulified_txt">${msgWrapper.dataset.qulifiedText}</b>`;
      }

      // console.log(cart.total_price, "Total Price");
      // console.log(shipping_value, "Threshold Price");
      //console.log(cart.items);

      // GWP Variables
      let $body = $('body');
      const isGWP = $body.data('codeless');
      const codeless_gwp_thresh = $body.data('code-thresh');
      const codeless_gwp_thresh_tiered = $body.data('code-thresh-tiered');
      const codeless_gwp_id_1 = $body.data('code-product-1') !== '' ? $body.data('code-product-1') : null;;
      const codeless_gwp_id_2 = $body.data('code-product-2') !== '' ? $body.data('code-product-2') : null;
      var checkGWPQty_1 = 0;
      var checkGWPQty_2 = 0;

      // CompareAt Vars
      var compare_price_total = 0;
      var total_quantity = 0;
      $.each(cart.items, function(index, item){
        $('.sf-cart-drawer .scd-item__original-price').eq(index).text(Shopify.formatMoney(item.final_line_price, '${{amount}}'));
        if(item.final_line_price != item.original_line_price && item.product_type !== "GWP"){
          let discountedItems = $('.sf-cart-drawer .cart-drawer__discounted-prices del');
          discountedItems.eq(index).text(Shopify.formatMoney(item.original_line_price, '${{amount}}')); 
        }
        
        if(item.id == codeless_gwp_id_1 && item.product_type === "GWP"){
          console.log('This is GWP 1ST CONDITION');
          checkGWPQty_1 = item.quantity;
          GWPItem = $('.sf-cart-drawer .scd__items .scd-item').eq(index);
          Shopify.propertiesGWP(GWPItem, item.product_description);
        }

        if(item.id == codeless_gwp_id_2 && item.product_type === "GWP"){
          console.log('This is GWP 2ND CONDITION');
          checkGWPQty_2 = item.quantity;
          GWPItem = $('.sf-cart-drawer .scd__items .scd-item').eq(index);
          Shopify.propertiesGWP(GWPItem, item.product_description);
        }
        ...
        $.ajax({
          ...
          success: function (itemData) {
              if (itemData) {
                  var item_comp = itemData;
                  if(item_comp.compare_at_price !== null && item_comp.compare_at_price > 0){
                    compare_price_total += (item_comp.compare_at_price/100) * item.quantity
                    total_quantity += item.quantity
                    $('.scd-item__price--discount').eq(index).text('$'+(item_comp.compare_at_price*item.quantity) / 100);
                  }else{
                    compare_price_total += parseFloat((item.original_line_price/100));
                  }
              }
          }
        });
      ...
      console.log(total_quantity, 'qanitity')
      console.log(compare_price_total, 'compare_price_total');

      if(compare_price_total > 0){
        $('.checkout_top del').removeClass('hidden').text(Shopify.formatMoney(compare_price_total*100, '${{ amount }}'));
        $('.checkout_bottom del').removeClass('hidden').text(Shopify.formatMoney(compare_price_total*100, '${{ amount }}'));
        $('.scd__subtotal-price:eq(1)').text(Shopify.formatMoney(compare_price_total*100, '${{ amount }}'));
        //$('.savings_on_front .saving_total').text('-$'+Math.round(compare_price_total - (cartTotal/100)));
        console.log("CARTTOTAL", compare_price_total, compare_price_total*100 - cartTotal)
        $('.savings_on_front .saving_total').text('-' + Shopify.formatMoney(compare_price_total*100 - cartTotal, '${{ amount }}'));
        //$('#grandTotal .scd__subtotal-price, .scd__subtotal-price:eq(0)').show().text('$'+Math.round((cartTotal)/100));
        $('#grandTotal .scd__subtotal-price, .scd__subtotal-price:eq(0)').show().text(Shopify.formatMoney(cartTotal, '${{ amount }}'));
        $('.scd__subtotal:eq(0) > *').css("font-weight","medium");
      }

      if(Math.round(compare_price_total - (cartTotal/100)) < 1){
        $('.scd__subtotal:eq(0) > *').css("font-weight","bold");
        $('.checkout_top del').addClass('hidden');
        $('.checkout_bottom del').addClass('hidden');
        $('.savings_on_front').hide()
      }

      //$('.scd__subtotal-price').eq(0).text('$'+Math.round((cartTotal-savings_price_final)/100));
      // typeof savings_price_final === 'undefined' ? savings_price_final = 0 : savings_price_final;
      // typeof saving_price_fixed === 'undefined' ? saving_price_fixed = 0 : saving_price_fixed;
      // if(savings_price_final > 1 ){
      //   $('.scd__subtotal-price:eq(0), .scd__subtotal-price:eq(2)').text('$'+savings_price_final);
      //   $('.checkout_top del').removeClass('hidden').text('$'+Math.round((cartTotal)/100 - savings_price_final));
      //   $('#grandTotal').show()
      // }else if(saving_price_fixed > 0){
      //   $('.scd__subtotal-price').eq(0).text('$'+Math.round((cartTotal)/100 - saving_price_fixed));
      //   $('.checkout_top del').addClass('hidden')
      // }else{
      //   $('.scd__subtotal-price').eq(0).text('$'+Math.round((cartTotal)/100));
      //   $('.checkout_top del').addClass('hidden');
      //   $('#grandTotal').hide()
      // }

      if (document.querySelector('.promoCodeWrap .savingsparent'))
      $('.savings_on_front').html(document.querySelector('.promoCodeWrap .savingsparent').outerHTML);

      
      // GWP Implimentation
      //console.log(cart, 'CART FOR GWP');
      console.log(cartTotal, 'CART TOTAL');

      if(cartTotal < codeless_gwp_thresh){
        if(sessionStorage['gwp_1'] === "remove"){
          sessionStorage.removeItem('gwp_1');
        }
      }
      if(cartTotal < codeless_gwp_thresh_tiered){
        if(sessionStorage['gwp_2'] === "remove"){
          sessionStorage.removeItem('gwp_2');
        }
      }

      if(isGWP && sessionStorage['gwp_1'] !== "remove" && codeless_gwp_id_1 !== null ){
        Shopify.codeless_gwp(cartTotal, codeless_gwp_thresh, codeless_gwp_id_1, checkGWPQty_1);
      }

      if(isGWP && sessionStorage['gwp_2'] !== "remove" && codeless_gwp_id_2 !== null ){
        Shopify.codeless_gwp(cartTotal, codeless_gwp_thresh_tiered, codeless_gwp_id_2, checkGWPQty_2);
      }
...
Shopify.codeless_gwp = function (cartTotal, codeless_gwp_thresh, codeless_gwp_id, checkGWPQty) {
  if(cartTotal >= codeless_gwp_thresh){
    console.log(`ADD ${codeless_gwp_thresh} VARIANT TO CART`, codeless_gwp_id);
    console.log(checkGWPQty, 'checkGWPQty');
    checkGWPQty == 0 && Shopify.addItem(codeless_gwp_id, 1);
    if(checkGWPQty == 1){
      getCart().then(cart =>{
        console.log(cart);
        var classfreeGWP = "";
        var GWPItem = "";
        $.each(cart.items, function(index, item){
          //console.log(item.product_type, index);
          if(item.product_type === 'GWP'){
            classfreeGWP = 'freeGWP_' + codeless_gwp_id;
            GWPItem = $(`.scd__items .scd-item[data-variant-id=${codeless_gwp_id}]`);
            GWPItem.addClass(classfreeGWP);
            //GWPItem.addClass('gift_item');
            //console.log(item.quantity, 'GIFT QUANTITY');
            //Shopify.propertiesGWP(GWPItem, item.product_description);
            if($(`.sf-cart-drawer .${classfreeGWP}.gift_item`).index() !== 0){ 
              $(`.sf-cart-drawer .${classfreeGWP}.gift_item`).insertBefore($('.sf-cart-drawer .scd-item').get(0));
            }
            $(`.sf-cart-drawer .${classfreeGWP}.gift_item a`).attr('href','javascript:void(0)');
            $(`.sf-cart-drawer .${classfreeGWP}.gift_item a`).removeClass('hover:underline');
            $(`.sf-cart-drawer .${classfreeGWP}.gift_item a`).css("pointer-events","none");
          }
        });
      });
    ...
    checkGWPQty > 0 ? Shopify.changeItem(codeless_gwp_id, 0) : '';
  }
}

Shopify.propertiesGWP = function (parentElem, desc){
  parentElem.find('.scd-item__quantity').addClass('invisible');
  parentElem.find('.scd-item__original-price').text('Free');
  // parentElem.find('.item_title .scd-item__variant').remove();
  // parentElem.find('.item_title').append(`<div class="scd-item__variant mb-0.5 flex"><span class="scd-item__variant-option mt-1">${desc}</span></div>`);
}
...
Shopify.onItemAdded = async (line_item, open_drawer = true) => {
  ...
  try {
    ...
    if (Cart) {
      await Cart.refreshCart();
      typeof handlePromoCode != 'undefined' ? handlePromoCode() : '';
      if (open_drawer) {
  ...
  return new Promise((resolve, reject) => {
    ...
    Promise.all(files.map(async file => {
      try {
        ...
        if (!window.loadedAssets.includes(`${name}.${type}`)) {
          window.loadedAssets.push(`${name}.${type}`);
          const {
            urls: {
              [name]: {
                url
...
function initOffsetmenuSlider() {
  // let offsetMenu = document.querySelector('.custom-offset-menu-dd-wrapper');
  // const productsBanner = offsetMenu.querySelector('.sf-mega-menu-products');
  // const id = productsBanner.dataset.id;
  ...
  //     slider = new window.spratlyTheme.Swiper(`.sf-slider-${id}`, {
  //       slidesPerView: 1,
  //       loop: false,
  //       autoplay: false,
  //       breakpoints: {
  ...
  //     });
  //     slider.map((val, idx)=>{
  //       const prevBtn = document.querySelectorAll(`#sf-slider-controls-${id} .sf-slider__controls-prev`)[idx];
  //       const nextBtn = document.querySelectorAll(`#sf-slider-controls-${id} .sf-slider__controls-next`)[idx];
  //       prevBtn && prevBtn.addEventListener('click', () => slider[idx].slidePrev());
  //       nextBtn && nextBtn.addEventListener('click', () => slider[idx].slideNext());
  //     });
  //   });
  // }
  //this.initProductsBanner(productsBanner);
}
...
register('footer', {
  onLoad: function () {
    src_default().register("screen and (max-width: 991px)", {
      match: () => {
        var _this$container;
...
register('slider', {
  onLoad: function () {
    ...
    this.loopStatus = this.container.dataset.enableLoop;
    this.lastVideo = null;
    ...
    if (isInViewport(this.container)) {
      ...
      this._initLightbox(_this, '[data-glightbox]');
    } else {
      if (window.__sfWindowLoaded) this._initSlider(_this);else window.addEventListener("load", () => {
        this._initSlider(_this);
        this._initLightbox(_this, '[data-glightbox]');
      });
    }
  ...
  _initLightbox: function (_this, selector) {
    loadAssets(['glightbox.css', 'glightbox.js']).then(() => {
      const lightboxElements = _this.container.querySelectorAll(selector);
      const elements = Array.from(lightboxElements).map(el => {
        ...
        switch (mediaType) {
          ...
          case 'external_video':
            ...
            const href = getVideoURL(el?.dataset?.external_id, el?.dataset?.host);
            return {
              href: href,
              type: 'video',
              source: el?.dataset?.host
            };
          case 'video':
            return {
              href: el?.dataset?.sourceUrl,
              type: 'video'
            };
          default:
            return {}
      ...
      if (window.spratlyLibs.GLightbox) {
        _this.lightbox = new window.spratlyLibs.GLightbox({
          elements: elements.filter(el => el.href),
          openEffect: 'fade',
          closeEffect: 'fade',
          draggable: false,
  ...
  _handleChange: function (swiper) {
    ...
    const mediaLightbox = slides[activeIndex].querySelector('.sf-slider-media-item');
    
    if (mediaLightbox) {
      const openButtons = slides[activeIndex].querySelectorAll('[data-open-lightbox]');
      if (openButtons.length > 0) {
        Array.from(openButtons).forEach(btn => 
          btn.addEventListener('click', (e) => {
            const index = Number(e.target?.parentElement?.dataset?.index) || 0;
    ...
    if (slideType === 'video_slide') {
      ...
      if (video) {
        ...
        let videos = slides[activeIndex].querySelectorAll('video');

        
        for (var i = 0; i < videos.length; i++) {
          if (videos[i].parentNode.parentNode.querySelector('.sf__video-controls')) {
          
            const videoSlide = videos[i];
            const pauseBtn = videos[i].parentNode.parentNode.querySelector('.sf__video-controls').children[0];
            pauseBtn.onclick = function()  {
              if (videoSlide.paused === true)  {
                videoSlide.play();
                if (pauseBtn){
                    pauseBtn.innerHTML = "<svg width='42' height='42' viewBox='0 0 42 42' fill='none'><path fill-rule='evenodd' clip-rule='evenodd' d='M42 21C42 32.598 32.598 42 21 42C9.40202 42 0 32.598 0 21C0 9.40202 9.40202 0 21 0C32.598 0 42 9.40202 42 21ZM13 12C13 11.4477 13.4477 11 14 11H18C18.5523 11 19 11.4477 19 12V29C19 29.5523 18.5523 30 18 30H14C13.4477 30 13 29.5523 13 29V12ZM25 11C24.4477 11 24 11.4477 24 12V29C24 29.5523 24.4477 30 25 30H29C29.5523 30 30 29.5523 30 29V12C30 11.4477 29.5523 11 29 11H25Z' fill='white'/></svg>";
                }
              } else {
                videoSlide.pause();
                if (pauseBtn){
                  pauseBtn.innerHTML = "<svg width='42' height='42' viewBox='0 0 42 42' fill='none'><path d='M21 0C9.39919 0 0 9.39919 0 21C0 32.6008 9.39919 42 21 42C32.6008 42 42 32.6008 42 21C42 9.39919 32.6008 0 21 0ZM30.7379 23.0323L15.8347 31.5847C14.4798 32.3468 12.871 31.4153 12.871 29.8065V12.1935C12.871 10.6694 14.4798 9.7379 15.8347 10.4153L30.7379 19.4758C32.1774 20.3226 32.1774 22.2702 30.7379 23.0323Z' fill='white'/></svg>";
              }
              }
            };
        }
...
// CONCATENATED MODULE: ./src/js/sections/custom-menu.js
...
register('custom-menu', {
  onLoad: function () {
    ...
    const links = container.querySelectorAll('.sf-menu__link > a');
    if (links.length > 0) {
      links.forEach(link => {
    ...
    //   initSlider();
    // } else {
    //   if (window.__sfWindowLoaded) initSlider()
...
AmyrisTheme.Notification = new Notification();
...
register('custom-content', {
  onLoad: function () {
    ...
    this.initLightbox(true);
    this.initCoverImage(true);
    ...
    if (parallaxEnabled && window.innerWidth > 767) {
      const bg = this.container.querySelector('.sf-custom__bg');
      if (!bg) return;
  ...
  initLightbox: function (enableImageZoom) {
    if (!enableImageZoom) return;

    const getExternalVideoId = (url, host) => {
      ...
      return host === "youtube" ? getYoutubeId(url) : getVimeoId(url);
    }

    loadAssets(['glightbox.css', 'glightbox.js']).then(() => {
      const mediaElements = this.container.querySelectorAll('[data-glightbox]');
      const elements = [...mediaElements]?.map(media => {
        switch (media?.dataset?.mediaType) {
          ...
          case 'external_video':
            // TODO: fix Youtube video on lightbox
            const href = getVideoURL(
              getExternalVideoId(media?.dataset?.sourceUrl, media?.dataset?.host),
              media?.dataset?.host
            );
            return {
              href: href,
              type: 'video',
              source: media?.dataset?.host
            };
          case 'video':
            return {
              href: media?.dataset?.sourceUrl,
              type: 'video'
            };
          default:
            return {};
      ...
      if (window.spratlyLibs && window.spratlyLibs.GLightbox) {
  ...
  initCoverImage: function () {
    ...
    let coverImages = Array.from(this.container.getElementsByClassName("sf-video__wrapper"));
    coverImages.forEach((element, index) => { 

      let videoCover = element.querySelector('.sf-video__cover');
      let videoContent = element.querySelector('.sf-video__inner');
      let videoTag = element.querySelector('.plyr__video-embed');
      ...
      function fadeOutEffect() {
        ...
        let fadeEffect = setInterval(function () {
          if (videoCover) {
            if (!videoCover.style.opacity) {
              videoCover.style.opacity = 1;
            }
            if (videoCover.style.opacity > 0) {
              videoTag.style.pointerEvents = "auto";
              videoCover.style.opacity -= 0.1;
            } else {
              clearInterval(fadeEffect);
            }
          }
          if (videoContent) {
            if (!videoContent.style.opacity) {
              videoContent.style.opacity = 1;
            }
            if (videoContent.style.opacity > 0) {
              videoTag.style.pointerEvents = "auto";
              videoContent.style.opacity -= 0.1;
            } else {
              clearInterval(fadeEffect);
            }
          }
      ...
      if(videoCover || videoContent) {
        const openButton = this.container.querySelector('[data-open-lightbox]');
        if (openButton) {
          openButton.addEventListener('click', (e) => {
            if (videoCover) fadeOutEffect();
            this.lightbox.open();
          });
        } else {
          if (videoCover || !video) {
  ...
  _initVideo: function (id) {
    const videoBlocks = this.container.querySelector('[data-video-block]');

    if (videoBlocks) {
      if (this.container.querySelector('.sf__video-controls')) {
        ...
        const videoSlide = this.container.querySelector('video');;
        const controlButton = this.container.querySelector('.sf__video-controls-pause');
        ...
        controlButton.addEventListener("click", (e) => {
          ...
          if (videoSlide.paused)  {
            videoSlide.play();
            if (controlButton){
              controlButton.querySelector("img").setAttribute("src", pauseButton);
            }
          } else {
            videoSlide.pause();
            if (controlButton){
              controlButton.querySelector("img").setAttribute("src", playButton);
            }
          }
        });
      };
...
;// CONCATENATED MODULE: ./src/js/sections/custom-list.js
...
;// CONCATENATED MODULE: ./src/js/sections/image-with-rich-text.js


register('image-with-rich-text', {
  onLoad: function () {
    let accList = this.container.querySelectorAll('.sf__accordio-blocks');
    accList.forEach(item => {
      new Accordion(item, {
...
function fetchCountDown(collectionID) {
  const appURL = window.spratlyThemeSettings.beautyAppURL ? `https://${window.spratlyThemeSettings.beautyAppURL}` : '';
  return new Promise((resolve, reject) => {
    var _window$Shopify;
...
// CONCATENATED MODULE: ./src/js/sections/recommendation-tabs.js
register('recommendation-tabs', {
  onLoad: function () {

    //main swiper - pagination per slide / tab
    const tabsOutSlider = this.container.querySelector('.sf__recommendation-tabs .mySwiper-recommendation-tabs-pages');
    var swiper = new Swiper(tabsOutSlider, {
      lazy: true,
      allowTouchMove: false,
      pagination: {
    ...
    //swiper slider inside each tab
    const tabInnerSlider = this.container.querySelectorAll('.sf__recommendation-tabs .swiper-container');
    for( i=0; i < tabInnerSlider.length; i++ ) {
      tabInnerSlider[i].classList.add('swiper-container-' + i);
...
;// Applied to all div.sf-collapsible 
...
const themeProducts = Shopify.themeProducts;
  ...
  constructor({
    ...
    _defineProperty(this, "initPlyrs", () => {
      ...
      if ((_this$domNodes = this.domNodes) !== null && _this$domNodes !== void 0 && (_this$domNodes$videos = _this$domNodes.videos) !== null && _this$domNodes$videos !== void 0 && _this$domNodes$videos.length) {
        loadAssets(['plyr.css', 'plyr.js']).then(() => {
          ...
          _ThemeEvent.emit('ON_PLYR_LOADED');
    ...
    _defineProperty(this, "initSlider", async () => {
      ...
      loadAssets(['swiper.css', 'swiper.js']).then(() => {
        ...
        this.navSlider = navSlider ? new Swiper(navSlider, {
          initialSlide,
          slidesPerView: 5,
          freeMode: true,
          spaceBetween: 10,
    ...
    _defineProperty(this, "initMobileSlider", () => {
      ...
      this.navSliderMobile = navSliderMobile ? Swiper(navSliderMobile, {
        initialSlide,
        slidesPerView: 3,
        spaceBetween: 10,
        loop: true,
      ...
      this.mobileSlider = new Swiper(slider, {
        initialSlide,
        slidesPerView: 1.2,
        centeredSlides: true,
        spaceBetween: 10,
    ...
    _defineProperty(this, "changeMediaByVariant", variant => {
      ...
      let gallery = document.getElementsByClassName("sf-product-media__desktop");  

      if(gallery){
      let galleryDivs = gallery[0];
      if(galleryDivs){
          let activImage = galleryDivs.querySelector("[data-index='0']");
          if(activImage){
            let img = activImage.querySelector('img');
    ...
    _defineProperty(this, "addEventToMainMedias", () => {
      addEventDelegate({
        ...
        handler: (e, media) => {
          if (!media.dataset.zoomable) {

            media.setAttribute("data-zoomable", true);
            setTimeout(() => media.removeAttribute("data-zoomable"), 500);
...
/* provided dependency */ var product_AmyrisTheme = __webpack_require__(4558)["AmyrisTheme"];
...
class Product {
  constructor(productForm, _options = {}) {
    ...
    _defineProperty(this, "setData", () => {
      ...
      this.hide_unavailable_product_options = AmyrisThemeSettings.hide_unavailable_product_options;
    ...
    _defineProperty(this, "initProductVariant", () => {
      ...
      if (productData && variantIdNode) {
        ...
        if (!currentVariantId && productData.selected_or_first_available_variant) {
    ...
    _defineProperty(this, "initStickyBar", () => {

      const headerHight = document.getElementById('header__wrapper');
      const productStickyAtc = [...document.querySelectorAll("#prod__sticky-atc")].filter(x => x.clientHeight > 0)[0];
      const addToCart = [...document.querySelectorAll("#sf-prod-template .add-to-cart")].filter(x => x.clientHeight > 0)[0];
      const notifyWhenAvailable = [...document.querySelectorAll("#klaviyoBackInStock")].filter(x => x.clientHeight > 0)[0];
      const headerDivHeight= headerHight.clientHeight || headerHight.offsetHeight || (headerHight.getBoundingClientRect()).height;

      if (productStickyAtc) { 
        let stickyBarTop = productStickyAtc.classList.contains('top-0');
        let stickyBarBottom = productStickyAtc.classList.contains('bottom-0');
      
        if ((stickyBarTop || stickyBarBottom) && !productStickyAtc.classList.contains("hidden")) {
          productStickyAtc.classList.add("hidden");
        }

        var atcScrollFunc = function() {
          const checkVisible = (elm) => {
            return !(elm.getBoundingClientRect().bottom < headerDivHeight);
          }

          if (stickyBarTop || stickyBarBottom) {
            if (checkVisible(addToCart || notifyWhenAvailable)) {
              if (!productStickyAtc.classList.contains("hidden")) {
                productStickyAtc.classList.add("hidden");
              }
              productStickyAtc.classList.remove("block");
            } else {
              if (!productStickyAtc.classList.contains("block")) {
                productStickyAtc.classList.add("block");
                if (stickyBarTop) {
                  productStickyAtc.style.marginTop = headerDivHeight + "px";
                }
              }
              productStickyAtc.classList.remove("hidden");
            }
          }
        }
      }

      window.addEventListener("scroll", atcScrollFunc);
    });
    ...
    _defineProperty(this, "handleQtyInputChange", e => {
      ...
      const quantitiesInput = document.querySelectorAll(".quantity-input__element");
      [...quantitiesInput].forEach(input => input.value = currentQty);
      const quantitiesText = document.querySelectorAll(".quantity-input .select-selected");
      [...quantitiesText].forEach(txtElement => txtElement.innerText = currentQty);
    ...
    _defineProperty(this, "removeAttribute", (ids, attribute) => {
      ids.forEach(id => {
        let ele = document.getElementById(id);
        if (ele && ele.hasAttribute(attribute)) {
          ele.removeAttribute(attribute);
        }
      })
    });

    _defineProperty(this, "addAttribute", (ids, attribute, value = "") => {
      ids.forEach(id => {
        let ele = document.getElementById(id);
        if (ele && !ele.hasAttribute(attribute)) {
    ...
    _defineProperty(this, "hideSoldoutAndUnavailableOptions", variant => {
      ...
      let notifyMeSubmit, userEmailInput, handleMessage;

        const notifyMeForm = document.querySelector('.prod__klaviyo-form');
        if (notifyMeForm) {
          notifyMeSubmit = notifyMeForm.querySelector('.klaviyo-form__submit');
          userEmailInput = notifyMeForm.querySelector(".klaviyo-form__email"); 
          handleMessage = notifyMeForm.querySelector('.kaviyo-form__message');
        }

        const label = document.querySelector('.notify-me-label');
        const stock = document.querySelector('.notify-me-stock');
        const formActions = document.querySelector('.product-form__actions');

        //send data to klaviyo - back in stock
        notifyMeSubmit && notifyMeSubmit.addEventListener('click', function (e) {
          e.preventDefault();
          ...
          fetch("https://a.klaviyo.com/onsite/components/back-in-stock/subscribe", {
            ...
            if (res.status == 200) {
              notifyMeSubmit.disabled = true;
              userEmailInput.disabled = true;
              handleMessage.style.display = "block";
              handleMessage.style.color = "#770000";
              handleMessage.style.marginBottom = "10px";
              handleMessage.innerHTML = "Thank you, we will notify you when the product is<br>back in stock.";
            } else {
              handleMessage.style.display = "block";
              handleMessage.style.color = "red";
              handleMessage.style.marginBottom = "10px";
              handleMessage.innerHTML = "Invalid email address. Please try again.";
            }
            return res.json();
          }).catch(
      ...
      optionNodes.forEach(optNode => {
        ...
        matchVariants = matchVariants.filter(Boolean);
                ...
                //add + remove class based on availability
                (variant.available) ? optNode.classList.remove('soldout-opt') : optNode.classList.add('soldout-opt');
        
                let divButtonsPDP = document.querySelector('.main-product__block-buy_buttons .add-to-cart');
                let divSitcky = document.querySelector('#sticky-atc-form .add-to-cart');
                let notifyMeHeader = document.querySelector('.notify-me-header');
                let notifyMe = document.querySelector('.prod__klaviyo-form');
        
                if (!variant.available) {
                  if (divButtonsPDP) divButtonsPDP.disabled = true;
                  if (divSitcky) divSitcky.disabled = true;
                  if (notifyMeHeader) notifyMeHeader.style.display = "flex";
                  if (notifyMe) notifyMeHeader.style.display = "flex";
                } else {
                  if (divButtonsPDP) divButtonsPDP.disabled = false;
                  if (divSitcky) divSitcky.disabled = false;
                  if (notifyMeHeader) notifyMeHeader.style.display = "none";
                  if (notifyMe) notifyMeHeader.style.display = "none";
                }

        if (optNode.dataset.selected == "true") { 

          const optionValuePDP = document.querySelectorAll('.main-product__block .product-option-item');
          const optionValueSticky = document.querySelectorAll('.sticky-variant-option .product-option-item');
    
          var index = optionNodes.indexOf(optNode);  
  
          //atc text in multiple variants
          const variantAtcText = optNode.dataset.atcText.split(", ");
          const variantAtcQuickView = document.querySelector('.sf-modal__wrapper .atc-text');
          const variantAtcPDP = document.querySelector('.main-product__block .atc-text');
          const variantAtcStickyBar = document.querySelector('.prod__sticky-atc .atc-text');

          //product badges - switch badge between variant - Refactor TODO
          const badges = optNode.dataset.badges.split(", ");
          const badgesBackground = optNode.dataset.badgesBackground.split(", ");
          const badgesTextColor = optNode.dataset.badgesText.split(", ");
          const badgeDivQuickView = document.querySelector('.sf-modal__wrapper .prod-badge');
          const badgeDivPDP = document.querySelector('.sf-prod-media__wrapper .prod-badge');

          //notify me - swap between coming soon and sold out - Refactor TODO
          const notifyText = optNode.dataset.notifyText.split(", ");
          const notifyTextQuickView = document.querySelector('.sf-modal__wrapper .notify-me-stock');
          const notifyTextPDP = document.querySelector('.main-product__block .notify-me-stock');

          if (optionValuePDP[index] && optionValueSticky[index]){
            document.querySelector('.main-product__block .product-option-item').dataset.selected = "false";
            document.querySelector('.main-product__block .product-option-item').checked = "false";
            document.querySelector('.sticky-variant-option .product-option-item').dataset.selected = "false";
            document.querySelector('.sticky-variant-option .product-option-item').checked = "false";


            optionValuePDP[index].checked = "true";
            optionValueSticky[index].checked = "true";
          } 

          for (let i = 0; i < variants.length; i++) {

            //product atc
            if (variants[i].id == variant.id && optNode.dataset.badges && (variantAtcQuickView || variantAtcPDP || variantAtcStickyBar )) {
              if (variantAtcQuickView) {
                variantAtcQuickView.innerHTML = variantAtcText[i];
              }
              if (variantAtcPDP) {
                variantAtcPDP.innerHTML = variantAtcText[i];
              }
              if (variantAtcStickyBar) {
                variantAtcStickyBar.innerHTML = variantAtcText[i];
                variantAtcStickyBar.classList.remove('disabled');
              }
            }

            //product badges
            if (variants[i].id == variant.id && optNode.dataset.badges && (badgeDivQuickView || badgeDivPDP)) {
              if (badgeDivQuickView) {
                badgeDivQuickView.innerHTML = badges[i];
                badgeDivQuickView.style.background = badgesBackground[i];
                badgeDivQuickView.style.color = badgesTextColor[i];
              }
              if (badgeDivPDP) {
                badgeDivPDP.innerHTML = badges[i];
                badgeDivPDP.style.background = badgesBackground[i];
                badgeDivPDP.style.color = badgesTextColor[i];
              }
            }

            //notify me - swap between coming soon and sold out
            if (variants[i].id == variant.id && optNode.dataset.notifyText && (notifyTextQuickView || notifyTextPDP)) {
              if (notifyTextQuickView) {
                notifyTextQuickView.innerHTML = notifyText[i];
              }
              if (notifyTextPDP) {
                notifyTextPDP.innerHTML = notifyText[i];
              }
            }
         }
        }
        ...
        // quickview elements
        const prodPriceEle = document.querySelector('.quick-view__info .prod__price');

        if (optNode.classList.contains('soldout-opt') && optNode.dataset.selected == "true" && optNode.dataset.notifyMe == "true") {

          formActions.style.display = "none";
          notifyMeForm.style.display = "block";
          
          //reset klaviyo form when select other variant
          handleMessage.innerHTML = "";
          handleMessage.style.display = "none";
          notifyMeSubmit.disabled = false;
          ...
          //send data to klaviyo - back in stock
          notifyMeSubmit.addEventListener('click', function (e) {
            e.preventDefault();
          ...
          //hide error message once email is edited
          userEmailInput.addEventListener("click", function() {
            handleMessage.style.display = "none";
          });
        }

        if (!optNode.classList.contains('soldout-opt') && optNode.dataset.selected == "true") {
          ...
          if (prodPriceEle) {
            prodPriceEle.style.textDecoration = "none";
          }
        }
      });
    });
    ...
    _defineProperty(this, "updateATCButtonByVariant", variant => {
      ...
      if (variant !== null && variant !== void 0 && variant.available) {
        ...
        if (atcTextNode) {
          ...
          atcTextNode.parentElement.removeAttribute('disabled');
          const formEle = atcTextNode.parentElement.parentElement.parentElement.querySelector('.form__input-wrapper');
          if (formEle) {
        ...
        if (atcTextNode) {
          if (typeof (variant === null || variant === void 0 ? void 0 : variant.available) === 'boolean') {
            ...
            atcTextNode.parentElement.setAttribute('disabled', '');
            if (atcTextNode.parentElement.parentElement.parentElement.querySelector('.form__input-wrapper')) atcTextNode.parentElement.parentElement.parentElement.querySelector('.form__input-wrapper').style.display = 'none';

            const $settings = atcTextNode.parentElement.parentElement.parentElement.querySelector('.add-to-cart');
            const atcTextComingSoon = atcTextNode.parentElement.dataset.productComingSoon;

            if (atcTextComingSoon == "true") {
              atcTextNode.innerText = comingSoon;
            }else{
...
product_AmyrisTheme.Product = Product;
product_AmyrisTheme.Products = new Products();
...
register('product-page', {
  ...
  onLoad: function () {
    ...
    addEventDelegate({
      context: this.container,
      selector: (window.__amyris_review_selector || '') + '.prod__accordion .jdgm-widget-actions-wrapper, .prod__accordion .spr-summary-actions-newreview',
      handler: e => {
...
;// CONCATENATED MODULE: ./src/js/sections/beauty-related-products.js

register('beauty-related-products', {
  onLoad: function () {
    var _window, _window$Beauty, _window$Beauty$beautyKit;

    if ((_window = window) !== null && _window !== void 0 && (_window$Beauty = _window.Beauty) !== null && _window$Beauty !== void 0 && (_window$Beauty$beautyKit = _window$Beauty.beautyKitSettings) !== null && _window$Beauty$beautyKit !== void 0 && _window$Beauty$beautyKit.productRecommendations) {
      var _window2, _window2$Beauty, _window2$Beauty$initP;

      (_window2 = window) === null || _window2 === void 0 ? void 0 : (_window2$Beauty = _window2.Beauty) === null || _window2$Beauty === void 0 ? void 0 : (_window2$Beauty$initP = _window2$Beauty.initProductRecommendations) === null || _window2$Beauty$initP === void 0 ? void 0 : _window2$Beauty$initP.call(_window2$Beauty);
    }
  }
});
...
/* provided dependency */ var cart_AmyrisTheme = __webpack_require__(4558)["AmyrisTheme"];
...
class Cart {
  constructor() {
    ...
    _defineProperty(this, "removeExtraGwp", async cart => {
      let updates = {};
      let changesMade = false;
      cart.items.forEach(pItem => {
        const { properties } = pItem;
        if (!properties) return;

        let remove = false;

        // Determine whether the item is a GWP from a show
        if (properties['_showId'] !== undefined && properties['_gwp'] !== undefined) {
          // Remove a liveshopping GWP if there is no other cart item from the same show
          remove = cart.items
            .find(item =>
              item.properties['_showId'] === properties['_showId'] &&
              item.properties['_gwp'] === undefined
            ) === undefined
        }
      ...
      if (changesMade) {
        this.cart = cart = await this.updateCart({updates: updates});
      }
    ...
    _defineProperty(this, "changeItemQty", async lineItem => {
      ...
      try {
        ...
        this.loading.start();
        const newCart = await this.removeExtraGwp(await this.changeCart(lineItem));
        this.cart = newCart;
    ...
    _defineProperty(this, "openCartDrawer", e => {
      ...
      const swiperPagination = document.querySelector(".product-recommendation-slider .swiper-pagination");
      if (swiperPagination && !swiperPagination.classList.contains("inited")) {
        swiperPagination.innerHTML = '';
        const swiper = new Swiper(".product-recommendation-slider", {
          slidesPerView: "auto",
          spaceBetween: 20,
          pagination: {
            el: swiperPagination,
      ...
      const menuClose = document.querySelector('.close-menu-mobile');
      if (menuClose) {
        menuClose.click();
      }
    ...
    window.beautyLabsCart.showMinicart = this.openCartDrawer;
    window.beautyLabsCart.onOutgoing = (cart) => {
      (async () => {
        const oldCart = cart;
        cart = await this.removeExtraGwp(cart);
        this.cart = cart;
        // If cart object reference has changed, that is because we removed something
        // so update the cart again inside the react app
        if (oldCart !== cart) {
          if (window.beautyLabsCart && window.beautyLabsCart.onIncoming) {
            window.beautyLabsCart.onIncoming(cart);
          }
    ...
    document.addEventListener('Rk:Added_To_Cart', function(event) {
      Shopify.onItemAdded(event.detail.items[0]);
    });
...
cart_AmyrisTheme.Cart = new Cart();
...
/* provided dependency */ var SearchProductItem_AmyrisThemeSettings = __webpack_require__(4558)["AmyrisThemeSettings"];
// eslint-disable-next-line no-unused-vars
...
/* harmony default export */ function SearchProductItem({
  ...
  if (Number(product.compare_at_price_max) > Number(product.price_min)) {
    ...
    comparePrice.innerHTML = formatMoney(product.compare_at_price_max, SearchProductItem_AmyrisThemeSettings.money_format);
  }

  const price = SearchProductItem_createElement("span", {
    className: "sf-currency"
  });
  price.innerHTML = formatMoney(product.price_min, SearchProductItem_AmyrisThemeSettings.money_format);
...
/* provided dependency */ var search_AmyrisTheme = __webpack_require__(4558)["AmyrisTheme"];
...
search_AmyrisTheme.Search = new Search();
...
/* provided dependency */ var compare_product_AmyrisTheme = __webpack_require__(4558)["AmyrisTheme"];
...
compare_product_AmyrisTheme.CompareProduct = new CompareProduct();
...
/* provided dependency */ var wishlist_AmyrisTheme = __webpack_require__(4558)["AmyrisTheme"];
...
wishlist_AmyrisTheme.Wishlist = new Wishlist();
...
/* provided dependency */ var app_AmyrisTheme = __webpack_require__(4558)["AmyrisTheme"];
...
function initTheme() {
  ...
  runHelpers();
  loadAllSections();
  app_AmyrisTheme.Products.initProductForms().catch(console.error);
}
...
var item = document.querySelectorAll('.offset-menu-item');
item.forEach(function(button, index){
    var checkIfDD = document.querySelectorAll('.offset-menu-item')[index].children[1];
    button.addEventListener('mouseenter', function(){ 
      if(checkIfDD != undefined && checkIfDD != null){
        var open = document.querySelector('.offset-menu-item.open');
        if (open != null)
          open.classList.remove('open');
          this.classList.add('open');
          document.children[0].classList.add('prevent-scroll');
          document.children[0].classList.add('is-menu-offset-open');
      }else{
        var open = document.querySelector('.offset-menu-item.open');
        if (open != null)
          open.classList.remove('open');
        document.children[0].classList.remove('prevent-scroll');
        document.children[0].classList.remove('is-menu-offset-open');
      }
    })
  
})

document.querySelectorAll('.offset-drawer-overlay, .close-menu').forEach(function(ele, idx) {
  ele.addEventListener('click', function(e) {
      var clickedEle = document.querySelector('.offset-menu-item.open');
      if (clickedEle != null)
          clickedEle.classList.remove('open');

      document.children[0].classList.remove('prevent-scroll'); 
      setTimeout(() => {
        document.querySelector('ul.offset-menu-parent > li').click();
        document.querySelectorAll('.offset-menu').forEach((elem, inx) => {
          elem.scrollTop = 0;
        })
      }, 500)
  })
...
var accordion = document.querySelectorAll('ul.offset-menu-parent>li, .offset-level-2 > li:not(.back-to-level-1)');
accordion.forEach(function(ele, idx) {
  ele.addEventListener('click', function(e) {
    //console.log(ele.children[0].pathname);
    if(ele.children[0].pathname.includes('void(0)') == true || ele.children[0].pathname.includes('#') == true){
...
// Desktop Search ---------------
const desktopSearch = {
  container: document.querySelector('.offsetField'),
  toggle: document.querySelector('[data-open-search-popup-offset-desktop] button'),
  closeBtn: document.querySelector('[data-close-search-offset]'),
  searchInit: function(){
    this.openField();
    this.closeField();
  },
  openField: function(){
    this.toggle.addEventListener('click', () => {
      ...
      document.querySelector('.offsetDesktopSearchField').focus();
    })
  },
  closeField: function(){
    document.addEventListener('click', ({ target }) => {
      if (!target.closest('[data-open-search-popup-offset-desktop]')) {
...
desktopSearch.searchInit();

// Mobile Search ---------------
const mobileSearch = {
  container: document.querySelector('[mobile-search]'),
  toggle: document.querySelector('[data-open-search-popup]'),
  closeBtn: document.querySelector('[data-close-search]'),
  searchInit: function(){
    this.openField();
    this.closeField();
  },
  openField: function(){
    this.toggle.addEventListener('click', () => {
      ...
      document.querySelector('[data-search-container] input[data-search-input]').focus();
    })
  },
  closeField: function(){
...
getCart().then(cart =>{
  Shopify.onCartUpdate(cart, false);
});

$(document).ready(function(){
  if($('body').data('codeless')){

    sessionStorage.gwp_1 && sessionStorage.removeItem('gwp_1');
    sessionStorage.gwp_2 && sessionStorage.removeItem('gwp_2');

    var gwpID_1 = $('body').data('codeProduct-1');
    var gwpID_2 = $('body').data('codeProduct-2');

    $(document).on('click', `.scd-item[data-variant-id=${gwpID_1}] .scd-item__remove`, function(){
      sessionStorage.setItem('gwp_1', 'remove');
      console.log('session gwp_1');
    });
    $(document).on('click', `.scd-item[data-variant-id=${gwpID_2}] .scd-item__remove`, function(){
      sessionStorage.setItem('gwp_2', 'remove');
      console.log('session gwp_2');
    })

  }
});

//redirect /cart to mini cart
...
//redirect to the right collection page
